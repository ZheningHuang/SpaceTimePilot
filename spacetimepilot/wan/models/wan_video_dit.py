...
def sinusoidal_embedding_1d(dim, position):
    # Handle both 1D and 2D position tensors (for batched inputs)
    original_shape = position.shape
    if position.dim() == 2:
        # Flatten 2D tensor to 1D for processing
        position_flat = position.flatten()
    else:
        position_flat = position
    
    sinusoid = torch.outer(position_flat.type(torch.float64), torch.pow(
        10000, -torch.arange(dim//2, dtype=torch.float64, device=position.device).div(dim//2)))
    x = torch.cat([torch.cos(sinusoid), torch.sin(sinusoid)], dim=1)
    
    # Reshape back to match original batch dimensions
    if len(original_shape) == 2:
        x = x.view(original_shape[0], original_shape[1], dim)
